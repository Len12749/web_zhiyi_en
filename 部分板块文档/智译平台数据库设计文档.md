...
| clerk_id | VARCHAR(255) | UNIQUE, NOT NULL | Clerk认证ID，外部身份标识 |
| email | VARCHAR(255) | NOT NULL | 用户邮箱 |
| points | INTEGER | NOT NULL, DEFAULT 20 | 积分余额，新用户默认20积分 |
...
|------|------|------|------|
| id | SERIAL | PRIMARY KEY | 自增主键 |
| user_id | VARCHAR(255) | NOT NULL, FOREIGN KEY | 关联用户ID (users.clerk_id) |
...
| points_earned | INTEGER | DEFAULT 5 | 获得积分，默认5分 |
| created_at | TIMESTAMP | DEFAULT now() | 创建时间 |
